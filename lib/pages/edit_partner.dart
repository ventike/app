...
class _EditPartnerPageState extends State<EditPartnerPage> {
  ...
  void save() {
    ...
    modifyPartner(userHash, id, name, description, email, phone, type, imageVal, individualFirstName, individualLastName, individualEmail, individualPhone, tags, resourceNamesVal, resourceTypesVal, resourceAmountsVal).then((res) {
      if (this.mounted) {
  ...
  void delete() {
    ...
    deletePartner(userHash, id).then((res) {
      if (this.mounted) {
        print(res.statusCode);
        print(res.body);
  ...
  Widget build(BuildContext context) {
    if (firstRun) {
      final args = ModalRoute.of(context)!.settings.arguments as EditPartnerArguments;
      ...
      email = args.email;
      phone = args.phone;
      image = args.image ?? placeholderPartnerImage;
      individualFirstName = args.individualFirstName;
      individualLastName = args.individualLastName;
      individualEmail = args.individualEmail;
      individualPhone = args.individualPhone;
      tags = args.tags;
      resources = args.resources;

      for (final tag in tags) {
        tagsContoller.text += tag["name"] + " ";
      }

      if (tagsContoller.text != "") {
        tagsContoller.text = tagsContoller.text.substring(0, tagsContoller.text.length - 1);
      }
      ...
      individualFirstNameController.text = args.individualFirstName;
      individualLastNameController.text = args.individualLastName;
      individualEmailController.text = args.individualEmail;
      individualPhoneController.text = args.individualPhone;

      resourcesCount = args.resources.length;

      for (final resource in args.resources) {
        resourceNames.add(TextEditingController(text: resource["name"]));
        resourceAmounts.add(TextEditingController(text: resource["amount"].toString()));
